...
public class DatenbankZugriffe extends JFrame implements ActionListener, WindowListener
{
	...
	private void InitializeComponent()
	{
		...
		miÖffnen = Globals.createMenuItem(menuDatei, Globals.MenuItemType.ITEM_PLAIN, "Datenbank öffnen ...", null, 'ö', null);
		miÖffnen.addActionListener(this);
		
		miSchliessen = Globals.createMenuItem(menuDatei, Globals.MenuItemType.ITEM_PLAIN, "Datenbank schliessen", null, 's', null);
		miSchliessen.addActionListener(this);	
		
		menuDatei.addSeparator();
		
		miBeenden = Globals.createMenuItem(menuDatei, Globals.MenuItemType.ITEM_PLAIN, "Beenden", null, 'B', null);
		...
		miPostleitzahlen = Globals.createMenuItem(menuStammdaten, Globals.MenuItemType.ITEM_PLAIN, "Postleitzahlen", null, 'P', null);
		miPostleitzahlen.addActionListener(this);
		
		menuAnsicht = new JMenu("Ansicht");
		menuStammdaten.setMnemonic('A');
		menuBar.add(menuAnsicht);
		
		miTreeview = Globals.createMenuItem(menuAnsicht, Globals.MenuItemType.ITEM_PLAIN, "Baumansicht", null, 'B', null);
		...
		miPostleitzahlenImportieren = Globals.createMenuItem(menuExtras, Globals.MenuItemType.ITEM_PLAIN, "Postleitzahlen importieren...", null, 'I', null);
		miPostleitzahlenImportieren.addActionListener(this);
	...
	private void openMySQLDatabase()
	{
		...
		connectionString = "jdbc:mysql://" + server + ":" + DBConnection.MYSQL_DEFAULTPORT + "/";
	...
	private class ReadFileIntoDatabase  implements Runnable
	  {
		...
		public ReadFileIntoDatabase(String Filename)
		{
			...
			progressWindow.setMaxValue(getRecordCount(Filename));
		...
		public void run()
		{
			...
			try (Scanner scanner = new Scanner(new FileInputStream(Filename)))
			{
				...
				while (scanner.hasNextLine())
				{
					...
					if (readCounter % 10 == 0)
						progressWindow.setMessage(String.format("Datensätze werden gelesen ...  [%s]", NumberFormat.getInstance().format(readCounter)));
