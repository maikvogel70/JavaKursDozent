...
public class Datum implements Comparable<Datum>
{
	...
	private static String[] arrWochentag = new String[]
	...
	/*
	 * Gauß hat eine Formel entwickelt, mit der man zu einem Datum bestehend aus Tag, Monat und Jahr den zugehörigen Wochentag ermitteln kann. Damit die
	 * untenstehende Formel richtige Ergebnisse liefert, muß das Datum zwischen dem 01.01.1582 und dem 31.12.3000 liegen. Die Berechnung erfolgt mit der
	 * folgenden von Gauß stammenden sogenannten Kalenderformel : Das zu behandelnde Datum sei in den Variablen tag, monat, jahr abgelegt.
	 *
	 * Damit werden zwei Hilfsgrößen h und k wie folgt berechnet :
	 * 
	 * Ist der Wert von monat <= 2, so ist h = monat + 12 und k = jahr - 1. Ist der Wert von monat > 2, so ist h = monat und k = jahr. Der Wochentag ergibt sich
	 * nun durch : wochentag = [ tag + 2*h + (3*h + 3) div 5 + k + k div 4 - k div 100 + k div 400 +1] mod 7
	 * 
	 * Dadurch erhält die Variable wochentag einen Wert zwischen 0 und 6 . Dabei bedeutet 0 = Sonntag , 1 = Montag , usw.
	 */
	...
	private int convertDateToNumber()
	{
		// Erstellt eine Ganzzahl im Format JJJJMMTT
	...
	public int compareTo(Datum that)
	{
		...
		int retValue = 0;
		//
		// if (this.convertDateToNumber() > that.convertDateToNumber())
		// retValue = 1;
		// else if (this.convertDateToNumber() < that.convertDateToNumber())
		// retValue = -1;
		//
		// return retValue;
		...
		retValue = Integer.compare(this.convertDateToNumber(), that.convertDateToNumber());
		if (retValue != 0)
			return retValue;

		// Wenn beide Vergleichswerte vom Typ DatumZeit sind, den Zeitanteil vergleichen
		if (this instanceof DatumZeit && that instanceof DatumZeit)
		{
			retValue = Integer.compare(((DatumZeit) this).convertTimeToNumber(), ((DatumZeit) that).convertTimeToNumber());
		}
		// Wenn nur das Argument vom Typ DatumZeit ist, wird für die eigene Klasse der Wert 0 (00:00:00) für
		// den Vergleich mit der Zeit verwendet.
		else if (that instanceof DatumZeit)
		{
			retValue = Integer.compare(0, ((DatumZeit) that).convertTimeToNumber());
		}
		// Wenn nur die eigene Klasse vom Typ DatumZeit ist, wird für das Argument der Wert 0 (00:00:00) für
		// den Vergleich mit der Zeit verwendet verwendet.
		else if (this instanceof DatumZeit)
		{
			retValue = Integer.compare(((DatumZeit) this).convertTimeToNumber(), 0);
		}
	...
	public boolean equals(Object that)
	{
		...
		if (this == that)
			...
			retValue = (compareTo((Datum) that) == 0);
	...
	public int hashCode()
	{
		// return convertDateToNumber();
		// oder
		return Integer.hashCode(convertDateToNumber());
