...
public class PLZTree extends JDialog implements ActionListener, MouseListener, WindowListener
{
	...
	private void createMenue()
	{
		...
		miErweitern = SwingUtil.createMenuItem(menuDatei, null, SwingUtil.MenuItemType.ITEM_PLAIN, this, "Alle Knoten erweitern", null, 'e', null);
		miReduzieren = SwingUtil.createMenuItem(menuDatei, null, SwingUtil.MenuItemType.ITEM_PLAIN, this, "Alle Knoten reduzieren", null, 'r', null);

		menuDatei.addSeparator();

		miBeenden = SwingUtil.createMenuItem(menuDatei, null, SwingUtil.MenuItemType.ITEM_PLAIN, this, "Beenden", null, 0, null);
	...
	private class MyJTree extends JTree
	{
		 ...
		 * handelt es sich um eine Methode des JTree, welche aufgerufen wird,
		 * wenn der Baum aufgeklappt wird. Diese Methode tut meines Wissens
		 * nichts, was im Normalfall benötigt wird. Allerdings benötigt sie eine
		 * Menge Rechenzeit und beansprucht Speicherplatz, wenn der Baum
		 * aufgeklappt werden soll. Deshalb wurde sie überschreiben und liefert
		 * einfach den Wert null zurück.
		 * 
		 */
	...
	private void selectedPath(TreePath path)
	{
		...
		// Es wurde kein Knoten angeklickt, sondern das Symbol zum Auf- bzwe.
		// Zuklappen
		if (path == null)
			return;
	...
	private class ReadPostleitzahlenIntoTree implements Runnable
	{
		...
		public void run()
		{
			...
			progressBar.setMaximum(recordCount);
			...
			try
			{
				...
				while (rSet.next())
				{
					...
					if (readCount % 10 == 0)
						statusBar.setText(String.format("Datensätze werden gelesen ...  [%d]", readCount));
			...
			{
				JOptionPane.showMessageDialog(null, "Fehler beim EInlesen der Postleitzahlen: " + ex.getMessage(), "Postleitzahlen",
